...
class ResidualBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_classes, stride=1, downsample=None):
        ...
        # FIXED: Create downsample layer if needed
        if downsample is not None:
            self.downsample = downsample
        elif stride != 1 or in_channels != out_channels:
            # Automatically create downsample layer
...
class RefineNet(nn.Module):
    ...
    def __init__(self, in_channels=3, base_channels=128, num_classes=10):
        ...
        # FIXED: stride=2 will automatically create downsample layers now
        ...
        # FIXED: Match channels for skip connections
        self.skip_conv2 = nn.Conv2d(base_channels * 4, base_channels * 4, 1)
        self.skip_conv1 = nn.Conv2d(base_channels * 2, base_channels * 2, 1)
    ...
    def forward(self, x, y):
        ...
        # Encoder path
        h1 = self.enc1(h, y)  # [B, 256, 16, 16]
        h2 = self.enc2(h1, y)  # [B, 512, 8, 8]
        h3 = self.enc3(h2, y)  # [B, 1024, 4, 4]

        # Decoder path with skip connections
        h = self.refine3(h3, y)  # [B, 512, 4, 4]
        h = self.upsample(h)  # [B, 512, 8, 8]
        h = h + self.skip_conv2(h2)  # Add skip connection with matching channels

        h = self.refine2(h, y)  # [B, 256, 8, 8]
        h = self.upsample(h)  # [B, 256, 16, 16]
        h = h + self.skip_conv1(h1)  # Add skip connection with matching channels

        h = self.refine1(h, y)  # [B, 128, 16, 16]
        h = self.upsample(h)  # [B, 128, 32, 32]

        # Output score
        score = self.conv_out(h)  # [B, 3, 32, 32]
