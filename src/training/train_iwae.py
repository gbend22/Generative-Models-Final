"""
IWAE Training Loop
"""
import torch
import torch.optim as optim
from tqdm import tqdm
import wandb
import os
import torchvision

from src.training.loss_iwae import iwae_loss
from src.utils import save_checkpoint


def train_iwae(model, dataloader, config, plot_callback=None):
    """
    Training loop for IWAE.

    Args:
        model: IWAE model
        dataloader: Training data loader
        config: IWAEConfig object
        plot_callback: Optional callback for Colab visualization
    """
    device = config.device
    model = model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=config.lr)
    k = config.n_importance_samples

    print(f"Starting IWAE training on {device}...")
    print(f"Number of importance samples (k): {k}")
    print(f"Latent dimension: {config.latent_dim}")
    print(f"Effective batch size: {config.batch_size} x {k} = {config.batch_size * k}")

    for epoch in range(config.n_epochs):
        model.train()
        total_loss_epoch = 0.0
        recon_loss_epoch = 0.0
        kl_loss_epoch = 0.0

        pbar = tqdm(dataloader, desc=f"Epoch {epoch + 1}/{config.n_epochs}")

        for batch_idx, (x, _) in enumerate(pbar):
            x = x.to(device)

            optimizer.zero_grad()
            x_recon, mu, log_var, z = model(x, k=k)

            total_loss, recon_loss, kl_loss = iwae_loss(
                x_recon, x, mu, log_var, z, k=k,
                recon_type=config.recon_type
            )

            total_loss.backward()
            optimizer.step()

            total_loss_epoch += total_loss.item()
            recon_loss_epoch += recon_loss.item()
            kl_loss_epoch += kl_loss.item()

            pbar.set_postfix({
                'loss': f'{total_loss.item():.2f}',
                'recon': f'{recon_loss.item():.2f}',
                'kl': f'{kl_loss.item():.2f}'
            })

            wandb.log({
                "train_loss": total_loss.item(),
                "recon_loss": recon_loss.item(),
                "kl_loss": kl_loss.item(),
            })

        n_batches = len(dataloader)
        avg_total = total_loss_epoch / n_batches
        avg_recon = recon_loss_epoch / n_batches
        avg_kl = kl_loss_epoch / n_batches

        print(f"Epoch {epoch + 1} - Loss: {avg_total:.4f}, "
              f"Recon: {avg_recon:.4f}, KL: {avg_kl:.4f}")

        wandb.log({
            "epoch": epoch + 1,
            "epoch_loss": avg_total,
            "epoch_recon_loss": avg_recon,
            "epoch_kl_loss": avg_kl,
        })

        # Sampling and visualization
        if (epoch + 1) % config.sample_interval == 0:
            print(f"Epoch {epoch + 1}: Generating samples...")

            model.eval()
            with torch.no_grad():
                # Generate samples from prior
                samples = model.sample(config.n_samples_to_show, device)

                # Reconstruct some training images
                x_sample = next(iter(dataloader))[0][:config.n_samples_to_show].to(device)
                x_recon_sample = model.reconstruct(x_sample)

            samples_grid = torchvision.utils.make_grid(
                samples.clamp(0, 1), nrow=4, padding=2
            )
            recon_grid = torchvision.utils.make_grid(
                torch.cat([x_sample, x_recon_sample.clamp(0, 1)], dim=0),
                nrow=config.n_samples_to_show,
                padding=2
            )

            wandb.log({
                "generated_samples": [wandb.Image(
                    samples_grid, caption=f"Epoch {epoch + 1}"
                )],
                "reconstructions": [wandb.Image(
                    recon_grid,
                    caption=f"Epoch {epoch + 1} (top: original, bottom: recon)"
                )]
            })

            if plot_callback:
                plot_callback(samples, epoch + 1)

            model.train()

        # Save checkpoint every 10 epochs
        if (epoch + 1) % 10 == 0:
            os.makedirs("checkpoints", exist_ok=True)
            save_checkpoint(
                model, optimizer, epoch,
                path=f"checkpoints/iwae_k{k}_epoch_{epoch + 1}.pth"
            )

    print("Training Complete.")
